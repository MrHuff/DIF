...
def adversarial_experiment(opt,model,dl_test):
    c = True
    while c:
        batch_class_source, c  = next(iter(dl_test))
    c=False
    while not c:
        batch_class_target, c  = next(iter(dl_test))
    d_img = torch.randn_like(batch_class_source,requires_grad=True)
    opt = torch.optim.Adam(d_img,lr=1e-3)
    ...
    save_image(rec_adversarial,opt.save_path+'/adversarial_experiments/rec_adversarial.jpg')
    ...
    save_image(d_img,opt.save_path+'/adversarial_experiments/noise.jpg')
